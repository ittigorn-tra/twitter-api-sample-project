...
class Helper:
  ...
  def parse_datetime(cls, dt,tz=None,**kwargs):
    ...
    if tz is None: tz = pytz.timezone(cls._settings.default_timezone)
    result = datetime.datetime.strptime(dt, fmt)
    if tz is not False: result = result.astimezone(tz)
    return result

  @classmethod
  def get_datetime(cls, dt_obj=None,fmt=None,tz=None, **kwargs):
    if 'dt_obj' in kwargs: dt_obj 	= kwargs['dt_obj']
    if 'fmt' 	in kwargs: fmt 		= kwargs['fmt']
    if 'tz' 	in kwargs: tz 		= kwargs['tz']
    return_obj = kwargs['return_obj'] if 'return_obj' in kwargs else False
    if fmt is None:
      if 'fmt' in kwargs:
        fmt = kwargs['fmt']
    if tz is None: 			tz 			= cls._settings.default_timezone
    if dt_obj is None:
    ...
    if return_obj:
      ...
      if fmt is None: 	fmt 		= '%Y-%m-%d %H:%M:%S.%f'
  ...
  def localize_utc_timestamp(cls, ts, return_obj=False):
    result = pytz.utc.localize(datetime.datetime.strptime(ts, '%Y-%m-%d %H:%M:%S.%f')).astimezone(pytz.timezone(cls._settings.default_timezone))
    if not return_obj:
  ...
  def localize_iso8601_utc_timestamp(cls, ts, return_obj=False):
    result = dateutil.parser.parse(ts).astimezone(pytz.timezone(cls._settings.default_timezone))
    if not return_obj:
      return result.strftime('%Y-%m-%d %H:%M:%S.%f')
    else:
      return result
