...
class Users(TwitterServices):
  ...
  def _format_user_search_results(cls, raw_results:dict) -> list:
    ...
    if len(tweets_df) > 0:
      ...
      # get user info
      user_info = raw_results.get('includes', {}).get('users', [])[0] if len(raw_results.get('includes', {}).get('users', [])) > 0 else {}
  ...
  def search_tweets_by_user_id(cls, user_id:str, limit:int, exclude_retweets:bool, exclude_replies:bool) -> list:
    ...
    # process excludes
    excludes = []
    if exclude_retweets: excludes.append('retweets')
    if exclude_replies:  excludes.append('replies')
    if len(excludes) > 0:  query_params['exclude'] = ','.join(excludes)
  ...
  def search_by_username(cls, username:str, limit:int, exclude_retweets:bool, exclude_replies:bool) -> list:
    ...
    results = cls.search_tweets_by_user_id(user_id=user_id, limit=limit, exclude_retweets=exclude_retweets, exclude_replies=exclude_replies) if user_id is not None else []
    return results
